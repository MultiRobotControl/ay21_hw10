...
odom5_msgs = select(bag,'Topic','/cora5/cora/sensors/p3d');
odom5_ts = timeseries(odom5_msgs,'Pose.Pose.Position.X','Pose.Pose.Position.Y', ...
    'Pose.Pose.Orientation.W','Pose.Pose.Orientation.X','Pose.Pose.Orientation.Y',...
    'Pose.Pose.Orientation.Z','Twist.Twist.Linear.X','Twist.Twist.Angular.Z');

odom6_msgs = select(bag,'Topic','/cora6/cora/sensors/p3d');
odom6_ts = timeseries(odom6_msgs,'Pose.Pose.Position.X','Pose.Pose.Position.Y', ...
    'Pose.Pose.Orientation.W','Pose.Pose.Orientation.X','Pose.Pose.Orientation.Y',...
    'Pose.Pose.Orientation.Z','Twist.Twist.Linear.X','Twist.Twist.Angular.Z');
...
cmd4_msgs = select(bag,'Topic','/cora4/cora/cmd_vel');
cmd4_ts = timeseries(cmd4_msgs,'Linear.X','Linear.Y','Linear.Z',...
        'Angular.X','Angular.Y','Angular.Z');
  
cmd5_msgs = select(bag,'Topic','/cora5/cora/cmd_vel');
cmd5_ts = timeseries(cmd5_msgs,'Linear.X','Linear.Y','Linear.Z',...
        'Angular.X','Angular.Y','Angular.Z');

cmd6_msgs = select(bag,'Topic','/cora6/cora/cmd_vel');
cmd6_ts = timeseries(cmd6_msgs,'Linear.X','Linear.Y','Linear.Z',...
        'Angular.X','Angular.Y','Angular.Z');
...
plot(odom4_ts.Data(:,1),odom4_ts.Data(:,2),'m'); hold on
plot(odom5_ts.Data(:,1),odom5_ts.Data(:,2),'c');
plot(odom6_ts.Data(:,1),odom6_ts.Data(:,2),'y'); 
...
legend('USV1','USV2','USV3','USV4','USV5','USV6','Rabbit Position', 'Location','best')
xlabel('X')
ylabel('Y')
grid on

%% 2) Plot Distance of each USV with Virtual Leader
X0 = rabbit_ts.Data(12:end,1);
Y0 = rabbit_ts.Data(12:end,2);
X1 = odom1_ts.Data(1:end,1);
Y1 = odom1_ts.Data(1:end,2);
X6 = odom6_ts.Data(11:end,1);
Y6 = odom6_ts.Data(11:end,2);

dist1 = sqrt((X1-X0).^2 + (Y1-Y0).^2);  
dist2 = sqrt((X6-X0).^2 + (Y6-Y0).^2); 

figure; clf();
% Plot the Data index corresponding to 'Pose.Pose.Position.X' & 'Pose.Pose.Position.Y'
plot(odom1_ts.Time(1:end),dist1,'r'); hold on
plot(odom6_ts.Time(11:end),dist2,'g'); 
title('Distance Between Each USV Cluster and Virtual Leader')
legend('USV Cluster 1','USV Cluster 2','Location','best')
xlabel('Time')
ylabel('Distance')
grid on